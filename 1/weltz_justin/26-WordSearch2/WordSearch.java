...
public class WordSearch {
	...
	int i = 0;
	while (i < numWords){
	    int index = r.nextInt(numWords);
	    while (answer.contains(wordList.get(index))){
		index = r.nextInt(numWords);
	    }
	    int count = 0;
	    boolean worked = false;
	    while (!worked && count < 30){
		worked = addWordRandomLoc(wordList.get(index));
		if (worked){
		    answer.add(wordList.get(index));
		}
		count = count + 1;
	    }  
	    i = i + 1;
