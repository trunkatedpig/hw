...
public class ArrayListBenchmark {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < times[0].length; i++) {System.out.println((long) Math.pow(10, i + 1) + ": " + times[0][i] + " ns/" + ((double) times[0][i] / 1000000) + " ms/" + ((double) times[0][i] / 1000000000) + " s");}
		
		System.out.println("\nTime to add to the beginning:");
		for (int i = 0; i < times[1].length; i++) {System.out.println((long) Math.pow(10, i + 1) + ": " + times[1][i] + " ns/" + ((double) times[1][i] / 1000000) + " ms/" + ((double) times[1][i] / 1000000000) + " s");}
		
		System.out.println("\nTime to remove from the end: ");
		for (int i = 0; i < times[2].length; i++) {System.out.println((long) Math.pow(10, i + 1) + ": " + times[2][i] + " ns/" + ((double) times[2][i] / 1000000) + " ms/" + ((double) times[2][i] / 1000000000) + " s");}
		
		System.out.println("\nTime to remove from the beginning:");
		for (int i = 0; i < times[3].length; i++) {System.out.println((long) Math.pow(10, i + 1) + ": " + times[3][i] + " ns/" + ((double) times[3][i] / 1000000) + " ms/" + ((double) times[3][i] / 1000000000) + " s");}
	}
}
